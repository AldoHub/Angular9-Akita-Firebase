...
export class PostComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    public getPost(){
      if(this.postsService.getPostEntity(this.currentId) !== undefined){
        this.postsService.getPostEntity(this.currentId).pipe(untilDestroyed(this))
        .subscribe(entity => {
        ...
        this.postsService.connect().pipe(
          untilDestroyed(this),
          map(async () => {
            await new Promise(resolve => {
              this.postsQuery.selectEntity(this.currentId).pipe(
                take(1),
                map(entity => {
                  return entity;
                })
  ...
  public async deletePost(postId: string){
    await this.postsService.remove(postId, this.currentPost.fileref);
